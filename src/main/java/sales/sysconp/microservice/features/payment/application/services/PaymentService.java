...
//        }
        ...
        // VALIDATE IF THE PAYMENT IS GREATER THAN THE INSTALLMENT VALUE
        List<PaymentModel> payments = installment.getPayments() != null ? installment.getPayments() : new ArrayList<>();
        double totalPaid = payments.stream().mapToDouble(PaymentModel::getValue).sum() + paymentCreateRequestDTO.getValue();
        double totalToBePaid = installment.getValue();
        if (installment.getDebt() != null) {
            totalToBePaid += installment.getDebt().getDebtValue();
        }
        if (totalPaid > totalToBePaid) {
            throw new IllegalArgumentException("You are trying to pay more than the installment value. You can only pay less.");
        }
        ...
        if (hasDebt) {
            if (process == PaymentProcessEnum.NO_DEBT) {
                throw new IllegalStateException("Installment has debt. Payment process must be PAY_DEBT or FORGIVE_DEBT.");
            }
        } else {
            if (process != PaymentProcessEnum.NO_DEBT) {
